...
{//This function has hard coded data.
	...
	list<FanPage>::iterator itrP = FanPageArr.begin();
	tempU = itrU;
	*itrP += (*itrU);
	*(++itrP) += (*(itrU));
	*(tempU) += *(++itrU);
	...
	*(itrU) += *(++tempU);
	itrU = userList.begin();
	itrP = FanPageArr.begin();
	(itrU)->insert_new_status(*(new Status(Date(25, 11, 2022), Time(12, 45), "Hello, My name is Netanel ")));
	(itrU)->insert_new_status(*(new Status(Date(25, 11, 2022), Time(14, 45), "What's up everyone?")));
	(++itrU)->insert_new_status(*(new Status(Date(27, 11, 2022), Time(13, 20), "Hello, My name is Amit  ")));
	(itrU)->insert_new_status(*(new Status(Date(27, 11, 2022), Time(16, 20), "What's up everyone? ")));
	(++itrU)->insert_new_status(*(new Status(Date(28, 11, 2022), Time(12, 45), "Hello, My name is Eden  ")));
	(itrU)->insert_new_status(*(new Status(Date(29, 11, 2022), Time(15, 45), "What's up everyone? ")));
	(itrP)->insert_new_status(*(new Status(Date(26, 11, 2022), Time(13, 13), "Everyone likes Food")));
	(itrP)->insert_new_status(*(new Status(Date(27, 11, 2022), Time(20, 12), "Follow this page for more recipes :)")));
	(++itrP)->insert_new_status(*(new Status(Date(28, 11, 2022), Time(15, 10), "What is the best game you played?")));
	(itrP)->insert_new_status(*(new Status(Date(29, 11, 2022), Time(20, 50), "Stay updated with the most recent news of the games world!")));
	(++itrP)->insert_new_status(*(new Status(Date(16, 12, 2022), Time(16, 20), "What is the best game you played?")));
	(itrP)->insert_new_status(*(new Status(Date(17, 12, 2022), Time(15, 40), "Stay updated with the most recent news of the games world!")));
}
...
{//This function add new member to facebook.
	...
	do
	{
		...
		index = findFriend(member->getName());
...
int  Facebook::findFriend(const string& name) const
...
{//This function search for fanPage in the fanPages array of facebook. 
 //If the fanPage in the array the function return the index else return -1.
	...
	string current_name;
	int i = 0, found = -1;
	list<FanPage>::const_iterator tr = FanPageArr.begin();
	list<FanPage>::const_iterator trEnd = FanPageArr.end();
...
{//This function add status to status array of requested member.
	...
	int index_member = findFriend(member_name);
...
{//This function add status to status fanPage of requested fanPage. 
	...
	list<FanPage>::iterator tr = FanPageArr.begin();
...
{//This function make connection between 2 members.
	...
	do
	{
		...
		friend1 = findFriend(friend_name);
		friend_name = std::move(input.getMemeberName());
		friend2 = findFriend(friend_name);
...
{//This function make the connection between member and fanPage.
	...
	do
	{
		...
		friend_name = std::move(input.getMemeberName());
		friendInd = findFriend(friend_name);
	...
	cin.ignore();
	cout << "\n******************************************\n";
	list<FanPage>::iterator trPage = FanPageArr.begin();
...
{//This function print all the members and fanPages in facebook.
	list<FanPage>::const_iterator trPage = FanPageArr.begin();
	list<User>::const_iterator trUser = userList.begin();
	list<FanPage>::const_iterator trPageEnd = FanPageArr.end();
...
{//This function print all the friends of requested member.
	...
	cin.ignore();
	int ind = findFriend(friend_name);
	list<User>::const_iterator trUser = userList.begin();
	while (ind == User::NOT_FOUND)
	{
		cout << "Could not find the requsted member!, please try again" << endl << endl;
		friend_name = std::move(input.getMemeberName());
		ind = findFriend(friend_name);
		cin.ignore();
	}
	for (int i = 0; i < ind; i++)
		++trUser;
...
{//This function compare between the amount of friends of 2 requested members.
	...
	do
	{
		...
		member1 = findFriend(member1_name);
		member2_name = std::move(input.getMemeberName());
		member2 = findFriend(member2_name);
...
{//This function compare between the amount of friends of 2 requested pages.
	...
	list<FanPage>::const_iterator itr1 = FanPageArr.begin();
	list<FanPage>::const_iterator itr2 = FanPageArr.begin();
